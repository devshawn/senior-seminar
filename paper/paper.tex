...
It is wise to note that this algorithm does not minimize the number of colored vertices. It can be beneficial to minimize the number of colored vertices (less cost) in certain applications such as building wireless networks. The conflict-free domination number for $k=2$, $\gamma_{CF}^2(G_3)$, is three even though we have four colored vertices in our example.
...
\label{sec:bounds}
Recall the map of the USA example given in the introduction. This is an example of a planar graph and illustrates a famous theorem associated with it. We present this theorem, called the four color theorem, in theorem \ref{thm:four}. It has been proved by multiple groups. The proof by computer that is widely accepted today can be found in \cite{robertson1997four}.

\begin{theorem} \label{thm:four}
Every loopless planar graph admits a vertex coloring with at most four distinct colors.
\end{theorem}

Due to theorem \ref{thm:four}, we immediately know that every planar graph is 4-colorable. This raises the question of whether there are planar graphs that require four colors or if fewer colors could suffice for all planar graphs. This leads us to the conflict-free variation of this theorem. We present this in theorem \ref{thm:four-cf}. It is given and proven in \cite{abel2017three}. In the next subsections, we will investigate this result.
...
\subsection{One or Two Colors}
\todo[inline]{To do still....}

\subsection{Three or More Colors}
...
\State Find a dominating set of $G$
\While{$V \setminus D \neq \emptyset$}
	\ForAll{$v \in V \setminus D$}
		\State Pick a vertex $u \in D$
...
%To prove theorem \ref{thm:approximate-cfds}, we use a reduction from \emph{k-coloring} to \emph{conflict-free k-coloring}. This reduction is out of the scope of this paper and can be found in \cite{abel2017three}. The reduction preserves the planarity of its input graph, so the resulting graph is still planar. We also know that 3-coloring a graph is NP-complete \cite{sharma2012new}.

Because the planar minimum dominating set problem is NP-hard \cite{garey2002computers}, we know that the conflict-free dominating set problem is NP-hard because we are finding a minimum dominating set. It follows that the corresponding decision problem, the k-conflict-free dominating set problem, is NP-complete.
...
There are many applications of the CFCP that benefit from the recent research into the field. It is easier to minimize frequency assignment for wireless networks and model RFID networks. Current research focuses on finding bounds and properties on specific cases such as planar graphs, interval graphs, types of hypergraphs, and more.
