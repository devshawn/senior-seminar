...
\begin{document}
  ...
  \begin{frame}
    ...
    \vspace{0.55cm}
      \begin{itemize}
        \item \textbf{Dominating set}: all vertices outside of the set must have distance 1 to some vertex within the set.
        \vfill
    ...
    \begin{textblock*}{15cm}(0.5cm, 2.1cm) % {block width} (coords)
      ...
      \only<4>{
        \begin{textblock*}{14cm}(1cm, 3.5cm) % {block width} (coords)
          \begin{theorem}
          A graph cannot be conflict-free colored with $k$ colors if it contains a complete graph of $k+2$ vertices or a tattered graph of $k+3$ vertices.
          \end{theorem}
        \end{textblock*}
      }
    \end{textblock*}
