...
\begin{document}
  ...
  \begin{frame}
    ...
    \only<1-2>{
      \begin{figure}[h]
        ...
        \caption*{Graphs $K_3$ and $K_4$, respectively}
      \end{figure}
    }
  ...
  \begin{frame}
    ...
    \begin{textblock*}{14cm}(1cm, 7.1cm) % {block width} (coords)
      A graph is a \textbf{minor} if it can be formed by deleting and/or contracting edges from its parent graph. To be conflict-free colored with 1 color, graph $G$ cannot contain $K_3$ or $K_4^{-3}$ as a minor.  \only<3>{$G$ contains both $K_3$ and $K_4^{-3}$ as a minor and thus {\color{red}\textbf{cannot}} be conflict-free colored with 1 color.}
    \end{textblock*}
